...
/// Built-in symbols with price feeds (keys of [`SYMBOL_MAP`]).
pub const WELL_KNOWN_SYMBOLS: &[&str] = &["eth", "btc", "usdc", "usdt", "dai", "weth"];

fn resolve_symbol(sym: &str) -> Option<String> {
    let s = sym.trim().to_ascii_lowercase();
    SYMBOL_MAP.iter().find(|(k, _)| *k == s.as_str()).map(|(_, v)| v.to_string())
}
